    ...
    <article className="min-h-screen bg-background text-foreground">
      ...
      <div className="relative h-[40vh] md:h-[50vh] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12 max-w-4xl mx-auto">
          <Link
            href="/blog"
            className="inline-flex items-center gap-1 text-sm text-zinc-300 hover:text-rose-gold transition-colors mb-4"
          >
            <ArrowLeft className="w-4 h-4" />
            Back to guides
          </Link>
          <p className="text-xs font-semibold text-rose-gold uppercase tracking-widest mb-2">
            {article.category}
          </p>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="prose prose-invert prose-rose max-w-none
          [&_h2]:text-2xl [&_h2]:font-bold [&_h2]:font-serif [&_h2]:text-gold-gradient [&_h2]:mt-12 [&_h2]:mb-4
          ...
          [&_a]:text-rose-gold [&_a]:hover:underline
          [&_hr]:border-card-border [&_hr]:my-10
          [&_blockquote]:border-l-4 [&_blockquote]:border-rose-gold [&_blockquote]:bg-card-bg [&_blockquote]:px-6 [&_blockquote]:py-4 [&_blockquote]:rounded-r-xl [&_blockquote]:my-6
          ...
          [&_.tip-box]:bg-card-bg [&_.tip-box]:border [&_.tip-box]:border-rose-gold/30 [&_.tip-box]:rounded-xl [&_.tip-box]:p-5 [&_.tip-box]:my-6
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {articles.map((a) => (
          <Link
            key={a.slug}
            href={`/blog/${a.slug}`}
            className="block rounded-2xl bg-card-bg border border-card-border hover:border-rose-gold/30 transition-all p-5"
          >
            <p className="text-[10px] font-semibold text-rose-gold uppercase tracking-widest mb-1">
              {a.category}
            </p>
            <h3 className="font-semibold text-foreground group-hover:text-rose-gold transition-colors mb-2">
              {a.title}
            </h3>
